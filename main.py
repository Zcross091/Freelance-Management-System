...
ADMIN_ID = 930518448268804096          # Your specific User ID (Yahya)
...
def init_db():
    ...
    ''')
    # Migration check: ensures the page_count column exists
    try:
...
async def complete(interaction: discord.Interaction, ticket_id: int, pages: int):
    ...
    if result[0] == 'PAID' or result[0] == 'SETTLED':
        conn.close()
        return await interaction.response.send_message(f"✅ Ticket #{ticket_id} is already completed.", ephemeral=True)
...
@bot.tree.command(name="payouts", description="Boss View: See current outstanding commission")
async def payouts(interaction: discord.Interaction):
    ...
    # Calculate current unpaid commission (only status = 'PAID')
    cursor.execute("SELECT SUM(page_count) FROM assignments WHERE status = 'PAID'")
    yahya_total = cursor.fetchone()[0] or 0
    
    # Breakdown by worker for the current unpaid batch
    cursor.execute("SELECT worker_name, SUM(page_count) FROM assignments WHERE status = 'PAID' GROUP BY worker_name")
    rows = cursor.fetchall()
    
    embed = discord.Embed(title="💰 Unpaid Commission Balance", color=discord.Color.orange())
    
    if not rows:
        embed.description = "Everything is settled! Current balance: **₹0**"
    else:
        for row in rows:
            name, pages = row
            embed.add_field(name=f"Worker: {name}", value=f"{pages} pages ➔ **₹{pages}**", inline=False)
        
        embed.add_field(name="──────────────", value=" ", inline=False)
        embed.add_field(name="💵 TOTAL OWED TO YOU", value=f"₹{yahya_total}", inline=True)
        embed.set_footer(text="Run /settle once you have received this cash.")
    
    await interaction.response.send_message(embed=embed)
    conn.close()

@bot.tree.command(name="settle", description="Yahya: Mark all current commissions as RECEIVED")
async def settle(interaction: discord.Interaction):
    ...
    cursor.execute("SELECT SUM(page_count) FROM assignments WHERE status = 'PAID'")
    to_settle = cursor.fetchone()[0] or 0
    
    if to_settle == 0:
        conn.close()
        return await interaction.response.send_message("ℹ️ No outstanding balance to settle.", ephemeral=True)
    
    # Move status from PAID to SETTLED
    cursor.execute("UPDATE assignments SET status = 'SETTLED' WHERE status = 'PAID'")
    conn.commit()
    conn.close()
    
    await interaction.response.send_message(f"✅ Successfully settled **₹{to_settle}**. Your balance is now **₹0**.")
